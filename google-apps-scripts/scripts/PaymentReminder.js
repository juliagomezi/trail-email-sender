function notifyUnpaidParticipants() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Preparem la plantilla HTML
    const tpl = HtmlService.createTemplateFromFile("PaymentReminderEmail");
...
function testNotifyUnpaidParticipants() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
  const data = sheet.getDataRange().getValues();
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!email || pagament || email != "eloiyerpes98@gmail.com") continue; // ignora si ja ha pagat o no hi ha email
